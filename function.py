...
def create_timestring(row):
    second_componet = (
        str(row["second"]) if row["second"] > 9 else "0" + str(row["second"])
    )
    minute_componet = (
        str(row["minute"]) if row["minute"] > 9 else "0" + str(row["minute"])
    )
    return minute_componet + ":" + second_componet
...
def get_event_dict(df, chosen_timestamp):
    ...
    timestamp_events["event_tag"] = (
        timestamp_events["player"] + ":" + timestamp_events["type"]
    )
...
#位置を描画関数
def create_plot(
    df, event_dict, chosen_timestamp, displayed_event, voronoi, home_team, away_team
):
    ...
    fig.set_facecolor("#f2f2f3")
    ax.patch.set_facecolor("#f2f2f3")
    ...
    if voronoi:
        team1, team2 = pitch.voronoi(
            [loc[0] for loc in frame["player_location"]],
            [loc[1] for loc in frame["player_location"]],
            frame["teammate"],
        )
...
def shot_freeze_frame(shot_df,tag,home_team,away_team,keeper_cone=False):
    ...
    for player in shot["shot_freeze_frame"]:
        if shot["team"] == home_team:
            ...
            color = home_color if player["teammate"] else home_color
    ...
    if keeper_cone:
        pitch.goal_angle(
            shot["location"][0], shot["location"][1], alpha=0.47, color="cyan",ax=ax
        )
        for player in shot["shot_freeze_frame"]:
